...
def suggest_trade_plan(price, atr, decision, df, style):
    ...
    if decision=="買い":
        if price>hi:
            tp=price+atr*atr_mult; sl=price-atr*atr_mult; is_break=True
        else:
            tp=hi*0.997; sl=price-abs(tp-price)/1.7
    elif decision=="売り":
        if price<lo:
            tp=price-atr*atr_mult; sl=price+atr*atr_mult; is_break=True
        else:
            tp=lo*0.997; sl=price+abs(tp-price)/1.7
    else:
        tp=sl=0
...
def run_backtest(df, style):
    ...
    if results:
        st.markdown("### 📊 バックテスト（過去100本）")
        wins=sum(1 for r in results if r["result"]=="win")
        losses=len(results)-wins
        total=len(results)
        win_rate=wins/total*100
        avg_pips=np.mean([r["pips"] for r in results])
        st.metric("勝率", f"{win_rate:.1f}% ({wins}/{total})")
        st.metric("期待値（平均pips）", f"{avg_pips:.1f}")
        with st.expander("▼ バックテスト詳細ログ", expanded=False):
            st.markdown("|本数|シグナル|結果|獲得pips|")
            st.markdown("|--|--|--|--|")
            for r in results:
                st.markdown(f"{r['index']} | {r['signal']} | {r['result']} | {r['pips']:.1f}")
    return results
...
if st.button("実行"):
    ...
    for tf in tf_map[style]:
        ...
        wt=tf_weights[tf]
        total_buy+=b*wt; total_sell+=s*wt
