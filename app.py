...
def suggest_trade_plan(price, atr, decision, tf, df):
    ...
    elif (style == "デイトレード" and tf == "4h") or (style == "スイング" and tf == "1day"):
...
def backtest(df_main, tf_main, df_ref):
    lower_tf = tf_lower.get(tf_main, tf_main)
    df_low = fetch_data(symbol, lower_tf)
    if df_low is None:
        return 0, 0, pd.DataFrame()
    df_low = calc_indicators(df_low)
    logs = []
    win = loss = 0
    for i in range(20, len(df_low)-1):
        sample = df_low.iloc[:i+1]
        ...
        hi = df_low["high"].iloc[i+1]
        lo = df_low["low"].iloc[i+1]
        result = "-"
        if signal == "買い":
            if hi >= tp and lo <= sl:
