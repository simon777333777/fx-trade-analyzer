...
if st.button("実行"):
    ...
    if decision != "待ち":
        ...
        st.markdown(f"• リスクリワード：{rr:.2f}")
    else:
        st.info("⏸ エントリー見送り")

    st.markdown("⸻\n### 📊 バックテスト（過去100本）")
    bt_result = run_backtest(main_df, style)
    wins = sum(1 for r in bt_result if r["result"] == "win")
    losses = sum(1 for r in bt_result if r["result"] == "lose")
    total = wins + losses
    win_rate = wins / total * 100 if total else 0
    avg_pips = np.mean([r["pips"] for r in bt_result]) if bt_result else 0
