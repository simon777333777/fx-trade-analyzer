...
if st.button("実行"):
    ...
    def suggest_trade_plan(df, direction):
        ...
        pips_tp = int(abs(tp - price) / pip_value)
        pips_sl = int(abs(price - sl) / pip_value)
        return price, tp, sl, rr, (pips_tp, pips_sl)

    # --- バックテスト ---
    def backtest(df, direction, pip_value):
        results = []
    ...
    if df_all is not None:
        ...
        if decision != "待ち":
            ...
            bt_results = backtest(df_all, decision, pip_value)
